...
public class BeforeVideoController {
    ...
    public ResponseBean addVideoInSchool(@RequestBody School school) {
        ...
        try {
            if (ArrayUtil.isNotEmpty(school.getCourseIds())) {
                ...
                bean.addError(ResponseBean.CODE_MESSAGE_ERROR,"请选择视频课程");
            }
            bean.addSuccess("添加成功");
        } catch (Exception e) {
            bean.addError(ResponseBean.CODE_MESSAGE_ERROR, "添加失败");
        }
    ...
    public ResponseBean delVideoSection(HttpServletRequest request, HttpServletResponse response) {
        ...
        videoService.delVideoSection(videoSectionId, videoSection.getVideoSectionSort(), videoSection.getVideoTotleSortId(), videoSection.getVideoId());
        bean.addSuccess();
        return bean;
    }
